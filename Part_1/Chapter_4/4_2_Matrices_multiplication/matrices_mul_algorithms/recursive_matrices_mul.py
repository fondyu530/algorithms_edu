...
def multiply_matrices_recursive(a: np.array, b: np.array) -> np.array:
    ...
    def _multiply_matrices_recursive(_a: np.array, _b: np.array) -> np.array:
        ...
        if size == 1:
            ...
            assign_sub_matrix(
                m=_c,
                m_sub=_multiply_matrices_recursive(a11, b11) + _multiply_matrices_recursive(a12, b21),
                start_row_ind=0,
                start_col_ind=0
            )
            assign_sub_matrix(
                m=_c,
                m_sub=_multiply_matrices_recursive(a11, b12) + _multiply_matrices_recursive(a12, b22),
                start_row_ind=0,
                start_col_ind=half_size
            )
            assign_sub_matrix(
                m=_c,
                m_sub=_multiply_matrices_recursive(a21, b11) + _multiply_matrices_recursive(a22, b21),
                start_row_ind=half_size,
                start_col_ind=0
            )
            assign_sub_matrix(
                m=_c,
                m_sub=_multiply_matrices_recursive(a21, b12) + _multiply_matrices_recursive(a22, b22),
                start_row_ind=half_size,
                start_col_ind=half_size
            )
        return _c
